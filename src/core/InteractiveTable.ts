...
export interface TableColumn {
  header: string;
  width: number;
  getValue: (item: any, index: number) => string | { content: string; href?: string };
}
  ...
  private drawTable(): void {
    ...
    currentPageData.forEach((item, localIndex) => {
      ...
      const rowData = this.options.columns.map(column => {
        const cellContent = column.getValue(item, globalIndex);
        return this.formatCellContent(cellContent, localIndex === this.selectedIndex);
      });
      
      // Add highlighting for selected row using colors
      if (localIndex === this.selectedIndex) {
        // Highlight selected row with yellow text
        const highlightedRowData = rowData.map((cell) => {
          if (typeof cell === 'object' && cell.content) {
            return {
              ...cell,
              content: colors.yellow(cell.content)
            };
          } else {
            return colors.yellow(cell.toString());
          }
        });
        table.push(highlightedRowData);
      } else {
        table.push(rowData);
      }
    });
